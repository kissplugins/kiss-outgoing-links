...
function ols_render_admin_page() {
    ...
    foreach ( $results as $r ) {
        if ( ! empty( $r['post_type'] ) ) {
            $post_type_filters[] = $r['post_type'];
        }
    }
    $post_type_filters = array_unique( $post_type_filters );
    ?>
    <div class="wrap">
        ...
        <?php if ( ! empty( $results ) ) : ?>
            ...
            <div id="ols-filter" style="margin-top:1rem;">
                <strong><?php esc_html_e( 'Filter by post type:', 'ols' ); ?></strong>
                <?php foreach ( $post_type_filters as $pt ) : ?>
                    <label style="margin-right:1rem;">
                        <input type="checkbox" value="<?php echo esc_attr( $pt ); ?>" checked="checked" />
            ...
            <table class="widefat fixed striped" id="ols-results" style="margin-top:1rem;">
                ...
                <?php foreach ( $results as $row ) : ?>
                    <tr data-post-type="<?php echo esc_attr( $row['post_type'] ); ?>">
 ...
 *     @type string $post_type Post type of the post containing the link.
 * }
 */
function ols_perform_scan() {
    ...
    // Pull only the fields we need to keep memory modest.
    $placeholders = implode( ',', array_fill( 0, count( $post_types ), '%s' ) );
    $query        = $wpdb->prepare( "SELECT ID, post_content, post_type FROM {$wpdb->posts} WHERE post_status = 'publish' AND post_type IN ($placeholders)", $post_types );
